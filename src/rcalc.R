...
if ( is.na(argv[1])){
    ...
    cat("cat iris.csv | Rscript rcalc.R -f 'write.table(summary(df),sep=\",\")' -d ','", "\n", sep="")
    cat("", "\n", sep="")
    ...
    cat("", "\n", sep="")
    cat("## ggplot2 another example: ggpairs", "\n", sep="")
    cat("1 | Rscript rcalc.R -f 'print( iris %>% dplyr::select(Sepal.Length, Petal.Length, Species) %>% ggpairs(mapping = aes(color = Species), lower = list(continuous = wrap(\"smooth\", size = 1.5, alpha = 0.8)), upper = list(continuous = wrap(\"cor\", size = 5)), diag = list(continuous = wrap(\"densityDiag\", alpha = 0.5)))+ggtitle(\"hoge\") )' -l 'tidyverse,GGally' --plot", "\n", sep="")
    cat("", "\n", sep="")
    cat("", "\n", sep="")
    ...
    cat("", "\n", sep="")
    cat("", "\n", sep="")
    cat("## pair plot using GGally::ggpairs() (ggplot2 and GGally)", "\n", sep="")
    cat("cat possum.csv | Rscript rcalc.R -f 'library(ggplot2);library(GGally);print(ggpairs(df))' -d ',' --plot", "\n", sep="")
    cat("", "\n", sep="")
    cat("cat iris.csv | Rscript rcalc.R -f 'library(ggplot2);library(GGally);print(df %>% GGally::ggpairs(ggplot2::aes(colour = df$species, alpha = 0.5)))' -d ',' --plot", "", "\n", sep="")
    cat("", "\n", sep="")
    cat("", "\n", sep="")
    cat("## pair plot using psych::pairs.panels", "\n", sep="")
    cat("cat iris.csv | Rscript rcalc.R -f 'library(psych);print(psych::pairs.panels(df))' -d ',' --plot", "\n", sep="")
    cat("", "\n", sep="")
    cat("", "\n", sep="")
    quit(status=1)
}
